...
import ac.jester.anticheat.checks.impl.badpackets.BadPacketsE;
import ac.jester.anticheat.checks.impl.badpackets.BadPacketsF;
import ac.jester.anticheat.checks.impl.badpackets.BadPacketsG;
import ac.jester.anticheat.checks.impl.badpackets.BadPacketsH;
import ac.jester.anticheat.checks.impl.elytra.ElytraC;
import ac.jester.anticheat.player.GrimPlayer;
import ac.jester.anticheat.utils.data.KnownInput;
import ac.jester.anticheat.utils.data.TrackerData;
...
public class PacketPlayerRespawn extends PacketListenerAbstract {
    ...
    public void onPacketSend(PacketSendEvent event) {
        if (event.getPacketType() == PacketType.Play.Server.UPDATE_HEALTH) {
            ...
            if (health.getHealth() <= 0) {
                player.latencyUtils.addRealTimeTask(player.lastTransactionSent.get(), () -> {
                    ...
                    player.packetStateData.showsDeathScreen = true;
                });
            } else {
        ...
        if (event.getPacketType() == PacketType.Play.Server.RESPAWN) {
            ...
            // Player respawned — death screen is gone
            player.packetStateData.showsDeathScreen = false;
