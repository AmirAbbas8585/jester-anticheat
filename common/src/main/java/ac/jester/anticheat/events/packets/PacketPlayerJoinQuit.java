...
public class PacketPlayerJoinQuit extends PacketListenerAbstract {
    ...
    public void onUserLogin(UserLoginEvent event) {
        ...
        // Permission-driven (LuckPerms), not op-based: anyone granted jester.alerts
        ...
        if (platformPlayer.hasPermission("jester.alerts", false)) {
            GrimAPI.INSTANCE.getAlertManager().toggleAlerts(platformPlayer, platformPlayer.hasPermission("jester.alerts.silent", false));
        }
        // Verbose is intentionally spammy (every flag) — keep it opt-in behind a
        // dedicated enable-on-join node, not auto-enabled by jester.verbose alone.
        if (platformPlayer.hasPermission("jester.verbose.enable-on-join", false) && platformPlayer.hasPermission("jester.verbose", false)) {
            GrimAPI.INSTANCE.getAlertManager().toggleVerbose(platformPlayer, platformPlayer.hasPermission("jester.verbose.silent", false));
        }
