...
public class SetbackTeleportUtil extends Check implements PostPredictionCheck {
     ...
     * @param yaw   - Player yaw from movement packet
     * @param pitch - Player pitch from movement packet
    ...
    public TeleportAcceptData checkTeleportQueue(double x, double y, double z, float yaw, float pitch) {
        ...
        while ((teleportPos = pendingTeleports.peek()) != null) {
            ...
            // When the server sends an absolute rotation (not relative), validate that the client
            // accepted it. Relative rotation teleports skip this check since any rotation is fine.
            ...
            if (player.lastTransactionReceived.get() == teleportPos.getTransaction() && Math.abs(clamped.getX() - x) <= threshold && closeEnoughY && Math.abs(clamped.getZ() - z) <= threshold && rotationMatches) {
                ...
                // Skip the flag during post-join / resource-pack load: a freezing or
                // still-loading client legitimately misses the spawn/setback teleport.
