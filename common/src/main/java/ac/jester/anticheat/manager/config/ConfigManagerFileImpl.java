...
public class ConfigManagerFileImpl implements ConfigManager, BasicReloadable {
    ...
    public void reload() {
        ...
        if (!initialized) {
            ...
            // The old separate punishments.yml is gone — per-check punishments
            // live in the "checks:" blocks of config.yml, and the optional grouped
            // "Punishments:" list there covers [log] history.
        }
        ...
        // Auto-migration: when an update ships new config keys, regenerate the
        // file from the bundled default (new keys + comments) while keeping the
        // user's existing values. Untouched when nothing is missing.
        ConfigMigrator.migrate(getConfigFile("config.yml"), "config/en.yml");
        ConfigMigrator.migrate(getConfigFile("messages.yml"), "messages/en.yml");
