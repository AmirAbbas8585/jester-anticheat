...
public class JumpPower {
    ...
    public static float getJumpPower(@NotNull GrimPlayer player) {
        float base = (float) player.compensatedEntities.self.getAttributeValue(Attributes.JUMP_STRENGTH) * getPlayerJumpFactor(player);
        double auraJumpMult = ExemptionProvider.safe().getJumpMultiplier(player);
        return auraJumpMult > 1.0 ? base * (float) auraJumpMult : base;
    }
