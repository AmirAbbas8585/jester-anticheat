...
public class JesterStopSpectating implements BuildableCommand {
        ...
        commandManager.command(
                commandManager.commandBuilder("jester", "jac")
                        ...
                        .optional("here", StringParser.stringParser(), SuggestionProvider.blocking((ctx, in) -> {
                            if (ctx.sender().hasPermission("jester.spectate.stophere")) {
    ...
    public void onStopSpectate(CommandContext<Sender> commandContext) {
        ...
        if (GrimAPI.INSTANCE.getSpectateManager().isSpectating(sender.getUniqueId())) {
            boolean teleportBack = string == null || !string.equalsIgnoreCase("here") || !sender.hasPermission("jester.spectate.stophere");
