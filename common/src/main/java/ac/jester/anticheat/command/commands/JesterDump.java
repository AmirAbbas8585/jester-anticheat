...
import ac.jester.anticheat.platform.api.PlatformPlugin;
...
import ac.jester.anticheat.utils.common.PropertiesUtil;
...
public class JesterDump implements BuildableCommand {
    ...
    private void handleDump(@NotNull CommandContext<Sender> context) {
        ...
        JesterLog.sendLogAsync(sender, generateDump(), string -> link = string, "text/yaml");
    }
    ...
    private static JsonObject getBuildInfo() {
        ...
        try {
            Properties properties = PropertiesUtil.readProperties(GrimAPI.INSTANCE.getClass(), "JesterAC.properties");
