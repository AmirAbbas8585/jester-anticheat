...
public class RotationPlace extends BlockPlaceCheck {
    ...
    public void onBlockPlace(final BlockPlace place) {
        ...
        // Only start CANCELLING placements after several recent misses. A single
        // miss (common with 1.8/ViaBackwards rapid-yaw) must not rubber-band legit
        // builds — real scaffold hacks miss every placement and climb past this fast.
    ...
    public void onPostFlyingBlockPlace(BlockPlace place) {
        ...
        // This can false with rapidly moving yaw in 1.8+/ViaBackwards clients, so
        // build a buffer over several misses. An ISOLATED miss (the common Via
        // false positive) only nudges the buffer and is NOT flagged/logged; only
        // the 2nd+ recent miss flags. Real scaffold misses every placement and so
        // climbs past this immediately.
        if (!hit) {
