// This file was designed and is an original check for SkyAC
...
public class Reach extends Check implements PacketCheck {
    ...
    /** How much extra reach to allow based on the player's movement toward the target. */
    ...
    private CheckResult checkReach(PacketEntity reachEntity, double x, double y, double z, boolean hasAttackRange, float itemMaxReach, float itemHitboxMargin, boolean isPrediction) {
        ...
        // 2.3.74: give a small allowance for forward movement toward target (reduces false positives)
        if (!isPrediction) {
    ...
    private double applyReachModifiers(SimpleCollisionBox targetBox, boolean hasAttackRange, float itemMaxReach, float itemHitboxMargin, boolean giveMovementThreshold) {
        ...
        // Sneak lenience: toggling sneak changes the eye height, and the server
        // processes the sneak state a tick behind the client, so for a tick the
        // hit geometry is computed from the "wrong" eye height — making a legit
        // hit measure slightly too far. A small extra margin while sneaking
        // absorbs that. Real reach (3.3+) is still far beyond this.
